...
function buildSubject(lead: Lead): string {
  const app = lead.application ?? 'general';
  return `🟢 Nuevo lead · ROHU Solutions · ${app} · ${lead.firstName}`;
}
...
function buildTextBody(lead: Lead): string {
  const lines = [
    ...
    '',
    `Aplicación de interés: ${lead.application ?? 'general'}`,
    `Plan de interés:       ${lead.planInterest ?? '—'}`,
    '',
    `Nombre:   ${lead.firstName}`,
    `Empresa:  ${lead.companyName}${lead.nit ? ` (NIT ${lead.nit})` : ''}`,
    `Ciudad:   ${lead.city}`,
    `Email:    ${lead.email}`,
    `WhatsApp: ${lead.whatsapp}`,
    `Tipo:     ${lead.businessType ?? '—'}`,
    `Usuarios: ${lead.numUsers ?? '—'}`,
  ];
...
function buildHtmlBody(lead: Lead): string {
  const safe = {
    ...
    businessType: lead.businessType ? escapeHtml(lead.businessType) : '—',
    numUsers: lead.numUsers ? escapeHtml(lead.numUsers) : '—',
    application: escapeHtml(lead.application ?? 'general'),
    planInterest: lead.planInterest ? escapeHtml(lead.planInterest) : '—',
  ...
  const waReplyUrl = buildWhatsAppUrl(
    e164,
    `Hola ${lead.firstName}, vi tu solicitud en ROHU Solutions. ¿Cuándo tienes 15 minutos para hablar?`
  );
  const mailtoUrl = `mailto:${lead.email}`;

  return `<!DOCTYPE html>
<html lang="es">
<head><meta charset="UTF-8"><title>Nuevo lead ROHU Solutions</title></head>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:24px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 10px 40px -15px rgba(30,58,138,0.35);">
        <tr>
          <td style="background:linear-gradient(135deg,#1E3A8A 0%,#06B6D4 100%);padding:28px 32px;color:#ffffff;">
            <div style="font-size:12px;letter-spacing:1px;text-transform:uppercase;opacity:0.85;margin-bottom:6px;">Nuevo lead · ROHU Solutions</div>
            <div style="font-size:22px;font-weight:700;">🟢 ${safe.firstName} — ${safe.companyName}</div>
        ...
        <tr>
          <td style="padding:24px 32px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;line-height:1.6;">
              <tr><td style="padding:6px 0;width:120px;color:#64748B;">Nombre</td><td style="padding:6px 0;"><strong>${safe.firstName}</strong></td></tr>
              <tr><td style="padding:6px 0;color:#64748B;">Empresa</td><td style="padding:6px 0;">${safe.companyName}${safe.nit ? ` · NIT ${safe.nit}` : ''}</td></tr>
              <tr><td style="padding:6px 0;color:#64748B;">Ciudad</td><td style="padding:6px 0;">${safe.city}</td></tr>
              <tr><td style="padding:6px 0;color:#64748B;">Email</td><td style="padding:6px 0;"><a href="${mailtoUrl}" style="color:#1E3A8A;">${safe.email}</a></td></tr>
              <tr><td style="padding:6px 0;color:#64748B;">WhatsApp</td><td style="padding:6px 0;">${safe.whatsapp}</td></tr>
              <tr><td style="padding:6px 0;color:#64748B;">Tipo</td><td style="padding:6px 0;">${safe.businessType}</td></tr>
              <tr><td style="padding:6px 0;color:#64748B;">Usuarios</td><td style="padding:6px 0;">${safe.numUsers}</td></tr>
              <tr><td style="padding:6px 0;color:#64748B;">Plan</td><td style="padding:6px 0;">${safe.planInterest}</td></tr>
            </table>
            ${safe.message ? `
