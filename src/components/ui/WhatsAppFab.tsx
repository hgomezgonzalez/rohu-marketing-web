 ...
 * of every page. Opens a pre-filled wa.me conversation whose message
 * adapts to the current route (corporate on /, app-specific on
 * /productos/<slug>). Tracks click_whatsapp_fab with the current page.
 */
export function WhatsAppFab() {
  const { phone } = getWhatsAppConfig();
  const pathname = usePathname();
  if (!phone) return null;
  const message = buildContextualWhatsAppMessage(pathname);
  ...
  return (
    <a
      ...
      onClick={() =>
        trackEvent(EVENTS.CLICK_WHATSAPP_FAB, {
          cta_location: 'fab',
